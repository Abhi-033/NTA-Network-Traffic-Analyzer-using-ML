...
def convert_to_csv(pcap_file_entry, output_csv):
    ...
    output_csv = "C:/Users/Abhi/OneDrive/Desktop/NTA/system/OUTPUTS/extracted_features.csv" # Change output path here
    if not pcap_file:
        messagebox.showerror("Error", "Please select a valid PCAP file.")
        return
    df = extract_features(packet, connection_stats)
...
def encode_and_save(input_entry, output_csv):
    ...
    output_csv = "C:/Users/Abhi/OneDrive/Desktop/NTA/system/OUTPUTS/encoded_features.csv" # Change output path here
    if not input_csv:
